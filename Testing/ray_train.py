# Save as: model_training/train_ray_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Ray ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_ray"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Ray
RAY_LETTER = 'ر'
...
# ========== DATA AUGMENTATION FOR RAY ==========
print(f"\n🔧 Augmenting Ray data for robustness...")

ray_mask = y_all == RAY_LETTER
X_ray_original = X_norm[ray_mask].copy()
y_ray_original = y_all[ray_mask].copy()

print(f"   Original Ray samples: {len(X_ray_original)}")

# Create augmented versions
augmented_X = [X_ray_original]
augmented_y = [y_ray_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_ray_original.shape).astype(np.float32)
X_noisy = np.clip(X_ray_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_ray_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_ray_original * scale, 0, 1)
...
X_ray_augmented = np.vstack(augmented_X)
y_ray_augmented = np.hstack(augmented_y)

print(f"   Augmented Ray samples: {len(X_ray_augmented)}")
...
y_other = y_all[~ray_mask]

# Take enough other samples to balance
n_ray = len(X_ray_augmented)
n_other = min(len(X_other), n_ray * 2)
...
# Combine
X = np.vstack([X_ray_augmented, X_other_bal])
y = np.hstack([np.ones(n_ray), np.zeros(n_other)])
...
print(f"   Ray (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_ray.h5'),
...
# Test on original Ray samples (not augmented)
ray_preds = model.predict(X_ray_original, verbose=0).flatten()
ray_detected = (ray_preds > 0.5).sum()
ray_rate = ray_detected / len(X_ray_original) * 100
print(f"\n   Original Ray detection: {ray_rate:.1f}% ({ray_detected}/{len(X_ray_original)})")
print(f"   Avg Ray confidence: {ray_preds.mean():.4f}")
print(f"   Min Ray confidence: {ray_preds.min():.4f}")
print(f"   Max Ray confidence: {ray_preds.max():.4f}")
...
info = {
    ...
    'ray_detection_rate': float(ray_rate),
...
print(f"   Ray Detection: {ray_rate:.1f}%")