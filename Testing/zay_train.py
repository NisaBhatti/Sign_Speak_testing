# Save as: model_training/train_zay_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Zay ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_zay"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Zay
ZAY_LETTER = 'ز'
...
# ========== DATA AUGMENTATION FOR ZAY ==========
print(f"\n🔧 Augmenting Zay data for robustness...")

zay_mask = y_all == ZAY_LETTER
X_zay_original = X_norm[zay_mask].copy()
y_zay_original = y_all[zay_mask].copy()

print(f"   Original Zay samples: {len(X_zay_original)}")

# Create augmented versions
augmented_X = [X_zay_original]
augmented_y = [y_zay_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_zay_original.shape).astype(np.float32)
X_noisy = np.clip(X_zay_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_zay_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_zay_original * scale, 0, 1)
...
X_zay_augmented = np.vstack(augmented_X)
y_zay_augmented = np.hstack(augmented_y)

print(f"   Augmented Zay samples: {len(X_zay_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~zay_mask]
y_other = y_all[~zay_mask]

# Take enough other samples to balance
n_zay = len(X_zay_augmented)
n_other = min(len(X_other), n_zay * 2)
...
# Combine
X = np.vstack([X_zay_augmented, X_other_bal])
y = np.hstack([np.ones(n_zay), np.zeros(n_other)])
...
print(f"   Zay (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_zay.h5'),
...
# Test on original Zay samples (not augmented)
zay_preds = model.predict(X_zay_original, verbose=0).flatten()
zay_detected = (zay_preds > 0.5).sum()
zay_rate = zay_detected / len(X_zay_original) * 100
print(f"\n   Original Zay detection: {zay_rate:.1f}% ({zay_detected}/{len(X_zay_original)})")
print(f"   Avg Zay confidence: {zay_preds.mean():.4f}")
print(f"   Min Zay confidence: {zay_preds.min():.4f}")
print(f"   Max Zay confidence: {zay_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'zay_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'zay_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'zay_detection_rate': float(zay_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Zay', 'Zay']
}

with open(os.path.join(OUTPUT_DIR, 'zay_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Zay Detection: {zay_rate:.1f}%")