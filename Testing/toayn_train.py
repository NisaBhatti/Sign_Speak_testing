# Save as: model_training/train_toayn_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Toayn ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_toayn"
...
# ========== DATA AUGMENTATION FOR TOAYN ==========
print(f"\n🔧 Augmenting Toayn data for robustness...")

toayn_mask = y_all == TOAYN_LETTER
X_toayn_original = X_norm[toayn_mask].copy()
y_toayn_original = y_all[toayn_mask].copy()

print(f"   Original Toayn samples: {len(X_toayn_original)}")

# Create augmented versions
augmented_X = [X_toayn_original]
augmented_y = [y_toayn_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_toayn_original.shape).astype(np.float32)
X_noisy = np.clip(X_toayn_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_toayn_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_toayn_original * scale, 0, 1)
...
for shift_x, shift_y in [(0.02, 0), (-0.02, 0), (0, 0.02), (0, -0.02)]:
    X_shifted = X_toayn_original.copy()
...
X_toayn_augmented = np.vstack(augmented_X)
y_toayn_augmented = np.hstack(augmented_y)

print(f"   Augmented Toayn samples: {len(X_toayn_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~toayn_mask]
y_other = y_all[~toayn_mask]

# Take enough other samples to balance
n_toayn = len(X_toayn_augmented)
n_other = min(len(X_other), n_toayn * 2)
...
# Combine
X = np.vstack([X_toayn_augmented, X_other_bal])
y = np.hstack([np.ones(n_toayn), np.zeros(n_other)])
...
print(f"   Toayn (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_toayn.h5'),
...
# Test on original Toayn samples (not augmented)
toayn_preds = model.predict(X_toayn_original, verbose=0).flatten()
toayn_detected = (toayn_preds > 0.5).sum()
toayn_rate = toayn_detected / len(X_toayn_original) * 100
print(f"\n   Original Toayn detection: {toayn_rate:.1f}% ({toayn_detected}/{len(X_toayn_original)})")
print(f"   Avg Toayn confidence: {toayn_preds.mean():.4f}")
print(f"   Min Toayn confidence: {toayn_preds.min():.4f}")
print(f"   Max Toayn confidence: {toayn_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'toayn_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'toayn_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'toayn_detection_rate': float(toayn_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Toayn', 'Toayn']
}

with open(os.path.join(OUTPUT_DIR, 'toayn_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Toayn Detection: {toayn_rate:.1f}%")