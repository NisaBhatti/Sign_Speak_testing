# Save as: testing/test_fe_robust.py
...
print("="*60)
print("SIGNSPEAK - Fe (ف) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_fe\fe_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_fe\fe_robust_info.json"
...
# ========== GET REFERENCE FE FROM DATABASE ==========
print("\n📊 Loading reference Fe landmarks...")
...
for row in all_data:
    ...
    if label == FE_LETTER:
        ...
        fe_samples.append(features)

if len(fe_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{FE_LETTER}' (U+0641)!")
    print(f"   The DB may not contain Fe samples.")
    exit(1)

fe_samples = np.array(fe_samples, dtype=np.float32)
fe_samples[:, 0::2] = fe_samples[:, 0::2] / 300.0
fe_samples[:, 1::2] = fe_samples[:, 1::2] / 300.0

ref_fe = fe_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(fe_samples)} Fe samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_fe.min(axis=0)
ref_max = ref_fe.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_fe_norm = (ref_fe - ref_min) / ref_range
...
cv2.namedWindow('Fe Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Fe Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 FE (ف) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Fe (follows your hand)")
print("   GREEN text = Fe detected!")
print("")
print("   How to sign Fe (ف):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_fe_norm - 0.5) * hand_size
            ...
            if is_fe:
                if smooth_pred > 0.8:
                    text = f"✅ FE (ف) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ FE (ف) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ FE (ف) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT FE - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT FE - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Fe"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Fe Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Fe", (45, h-13),
        ...
        ref_ghost = (ref_fe_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Fe Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"fe_test_{frame_count}.png"
