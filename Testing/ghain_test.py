# Save as: testing/test_ghain_robust.py
...
print("="*60)
print("SIGNSPEAK - Ǧain (غ) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_ghain\ghain_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_ghain\ghain_robust_info.json"
...
# ========== GET REFERENCE GHAIN FROM DATABASE ==========
print("\n📊 Loading reference Ǧain landmarks...")
...
for row in all_data:
    ...
    if label == GHAIN_LETTER:
        features = [float(row[i]) for i in range(1, 43)]
        ghain_samples.append(features)

if len(ghain_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{GHAIN_LETTER}' (U+063A)!")
    print(f"   The DB may not contain Ǧain samples.")
    exit(1)

ghain_samples = np.array(ghain_samples, dtype=np.float32)
ghain_samples[:, 0::2] = ghain_samples[:, 0::2] / 300.0
ghain_samples[:, 1::2] = ghain_samples[:, 1::2] / 300.0

ref_ghain = ghain_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(ghain_samples)} Ǧain samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_ghain.min(axis=0)
ref_max = ref_ghain.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_ghain_norm = (ref_ghain - ref_min) / ref_range
...
cv2.namedWindow('Ghain Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Ghain Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 ǦAIN (غ) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Ǧain (follows your hand)")
print("   GREEN text = Ǧain detected!")
print("")
print("   How to sign Ǧain (غ):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_ghain_norm - 0.5) * hand_size
            ...
            if is_ghain:
                if smooth_pred > 0.8:
                    text = f"✅ ǦAIN (غ) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ ǦAIN (غ) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ ǦAIN (غ) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT ǦAIN - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT ǦAIN - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Ǧain"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Ǧain Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Ǧain", (45, h-13),
        ...
        ref_ghost = (ref_ghain_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Ghain Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"ghain_test_{frame_count}.png"
