# Save as: testing/test_hamza_robust.py
...
print("="*60)
print("SIGNSPEAK - Hamzā (ء) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_hamza\hamza_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_hamza\hamza_robust_info.json"
...
# ========== GET REFERENCE HAMZA FROM DATABASE ==========
print("\n📊 Loading reference Hamzā landmarks...")
...
for row in all_data:
    ...
    if label == HAMZA_LETTER:
        features = [float(row[i]) for i in range(1, 43)]
        hamza_samples.append(features)

if len(hamza_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{HAMZA_LETTER}' (U+0621)!")
    print(f"   The DB may not contain Hamzā samples.")
    exit(1)

hamza_samples = np.array(hamza_samples, dtype=np.float32)
hamza_samples[:, 0::2] = hamza_samples[:, 0::2] / 300.0
hamza_samples[:, 1::2] = hamza_samples[:, 1::2] / 300.0

ref_hamza = hamza_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(hamza_samples)} Hamzā samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_hamza.min(axis=0)
ref_max = ref_hamza.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_hamza_norm = (ref_hamza - ref_min) / ref_range
...
cv2.namedWindow('Hamza Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Hamza Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 HAMZĀ (ء) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Hamzā (follows your hand)")
print("   GREEN text = Hamzā detected!")
print("")
print("   How to sign Hamzā (ء):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_hamza_norm - 0.5) * hand_size
            ...
            if is_hamza:
                if smooth_pred > 0.8:
                    text = f"✅ HAMZĀ (ء) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ HAMZĀ (ء) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ HAMZĀ (ء) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT HAMZĀ - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT HAMZĀ - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Hamzā"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Hamzā Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Hamzā", (45, h-13),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.circle(frame, (220, h-20), 6, (0, 255, 0), -1)
            cv2.putText(frame, "Your Hand", (235, h-13),
        ...
        ref_ghost = (ref_hamza_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Hamza Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"hamza_test_{frame_count}.png"
