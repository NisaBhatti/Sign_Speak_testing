# Save as: testing/test_bari_ye_robust.py
...
print("="*60)
print("SIGNSPEAK - Baṛī Ye Robust Detection (Clamped Reference)")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_bari_ye\bari_ye_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_bari_ye\bari_ye_robust_info.json"
...
# ========== GET REFERENCE BARI YE FROM DATABASE ==========
print("\n📊 Loading reference Baṛī Ye landmarks...")
...
for row in all_data:
    ...
    if label == BARI_YE_LETTER:
        features = [float(row[i]) for i in range(1, 43)]
        bari_ye_samples.append(features)

bari_ye_samples = np.array(bari_ye_samples, dtype=np.float32)
bari_ye_samples[:, 0::2] = bari_ye_samples[:, 0::2] / 300.0
bari_ye_samples[:, 1::2] = bari_ye_samples[:, 1::2] / 300.0

ref_bari_ye = bari_ye_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(bari_ye_samples)} Baṛī Ye samples")

# Normalize reference to 0-1 bounding box
ref_min = ref_bari_ye.min(axis=0)
ref_max = ref_bari_ye.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_bari_ye_norm = (ref_bari_ye - ref_min) / ref_range
...
cv2.namedWindow('Bari Ye Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Bari Ye Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 BAṚĪ YE (ے) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Baṛī Ye (follows, stays on-screen)")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_bari_ye_norm - 0.5) * hand_size
            ...
            if is_bari_ye:
                if smooth_pred > 0.8:
                    text = f"✅ BAṚĪ YE (ے) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ BAṚĪ YE (ے) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ BAṚĪ YE (ے) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT BAṚĪ YE - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT BAṚĪ YE - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Baṛī Ye"
                    color = (0, 165, 255)
            
            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Baṛī Ye Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Baṛī Ye", (45, h-13),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.circle(frame, (230, h-20), 6, (0, 255, 0), -1)
            cv2.putText(frame, "Your Hand", (245, h-13),
        ...
        ref_ghost = (ref_bari_ye_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Bari Ye Detection - Robust', frame)
    
    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"bari_ye_test_{frame_count}.png"
