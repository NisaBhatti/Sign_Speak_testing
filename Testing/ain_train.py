# Save as: model_training/train_ain_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Ain ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_ain"
...
# ========== DATA AUGMENTATION FOR AIN ==========
print(f"\n🔧 Augmenting Ain data for robustness...")

ain_mask = y_all == AIN_LETTER
X_ain_original = X_norm[ain_mask].copy()
y_ain_original = y_all[ain_mask].copy()

print(f"   Original Ain samples: {len(X_ain_original)}")

# Create augmented versions
augmented_X = [X_ain_original]
augmented_y = [y_ain_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_ain_original.shape).astype(np.float32)
X_noisy = np.clip(X_ain_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_ain_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_ain_original * scale, 0, 1)
...
X_ain_augmented = np.vstack(augmented_X)
y_ain_augmented = np.hstack(augmented_y)

print(f"   Augmented Ain samples: {len(X_ain_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~ain_mask]
y_other = y_all[~ain_mask]

# Take enough other samples to balance
n_ain = len(X_ain_augmented)
n_other = min(len(X_other), n_ain * 2)
...
# Combine
X = np.vstack([X_ain_augmented, X_other_bal])
y = np.hstack([np.ones(n_ain), np.zeros(n_other)])
...
print(f"   Ain (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_ain.h5'),
...
# Test on original Ain samples (not augmented)
ain_preds = model.predict(X_ain_original, verbose=0).flatten()
ain_detected = (ain_preds > 0.5).sum()
ain_rate = ain_detected / len(X_ain_original) * 100
print(f"\n   Original Ain detection: {ain_rate:.1f}% ({ain_detected}/{len(X_ain_original)})")
print(f"   Avg Ain confidence: {ain_preds.mean():.4f}")
print(f"   Min Ain confidence: {ain_preds.min():.4f}")
print(f"   Max Ain confidence: {ain_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'ain_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'ain_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'ain_detection_rate': float(ain_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Ain', 'Ain']
}

with open(os.path.join(OUTPUT_DIR, 'ain_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Ain Detection: {ain_rate:.1f}%")
