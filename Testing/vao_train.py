# Save as: model_training/train_vao_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Vao ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_vao"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Vao (Waw)
VAO_LETTER = 'و'
...
# ========== DATA AUGMENTATION FOR VAO ==========
print(f"\n🔧 Augmenting Vao data for robustness...")

vao_mask = y_all == VAO_LETTER
X_vao_original = X_norm[vao_mask].copy()
y_vao_original = y_all[vao_mask].copy()

print(f"   Original Vao samples: {len(X_vao_original)}")

# Create augmented versions
augmented_X = [X_vao_original]
augmented_y = [y_vao_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_vao_original.shape).astype(np.float32)
X_noisy = np.clip(X_vao_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_vao_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_vao_original * scale, 0, 1)
...
X_vao_augmented = np.vstack(augmented_X)
y_vao_augmented = np.hstack(augmented_y)

print(f"   Augmented Vao samples: {len(X_vao_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~vao_mask]
y_other = y_all[~vao_mask]

# Take enough other samples to balance
n_vao = len(X_vao_augmented)
n_other = min(len(X_other), n_vao * 2)
...
# Combine
X = np.vstack([X_vao_augmented, X_other_bal])
y = np.hstack([np.ones(n_vao), np.zeros(n_other)])
...
print(f"   Vao (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_vao.h5'),
...
# Test on original Vao samples (not augmented)
vao_preds = model.predict(X_vao_original, verbose=0).flatten()
vao_detected = (vao_preds > 0.5).sum()
vao_rate = vao_detected / len(X_vao_original) * 100
print(f"\n   Original Vao detection: {vao_rate:.1f}% ({vao_detected}/{len(X_vao_original)})")
print(f"   Avg Vao confidence: {vao_preds.mean():.4f}")
print(f"   Min Vao confidence: {vao_preds.min():.4f}")
print(f"   Max Vao confidence: {vao_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'vao_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'vao_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'vao_detection_rate': float(vao_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Vao', 'Vao']
}

with open(os.path.join(OUTPUT_DIR, 'vao_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Vao Detection: {vao_rate:.1f}%")