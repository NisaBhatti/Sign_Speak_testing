# Save as: testing/test_mim_robust.py
...
print("="*60)
print("SIGNSPEAK - Mīm (م) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_mim\mim_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_mim\mim_robust_info.json"
...
# ========== GET REFERENCE MIM FROM DATABASE ==========
print("\n📊 Loading reference Mīm landmarks...")
...
for row in all_data:
    ...
    if label == MIM_LETTER:
        ...
        mim_samples.append(features)

if len(mim_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{MIM_LETTER}' (U+0645)!")
    print(f"   The DB may not contain Mīm samples.")
    exit(1)

mim_samples = np.array(mim_samples, dtype=np.float32)
mim_samples[:, 0::2] = mim_samples[:, 0::2] / 300.0
mim_samples[:, 1::2] = mim_samples[:, 1::2] / 300.0

ref_mim = mim_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(mim_samples)} Mīm samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_mim.min(axis=0)
ref_max = ref_mim.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_mim_norm = (ref_mim - ref_min) / ref_range
...
cv2.namedWindow('Mim Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Mim Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 MĪM (م) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Mīm (follows your hand)")
print("   GREEN text = Mīm detected!")
print("")
print("   How to sign Mīm (م):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_mim_norm - 0.5) * hand_size
            ...
            if is_mim:
                if smooth_pred > 0.8:
                    text = f"✅ MĪM (م) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ MĪM (م) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ MĪM (م) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT MĪM - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT MĪM - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Mīm"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Mīm Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Mīm", (45, h-13),
        ...
        ref_ghost = (ref_mim_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Mim Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"mim_test_{frame_count}.png"
