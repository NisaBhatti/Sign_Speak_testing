# Save as: testing/test_quaaf_robust.py
...
print("="*60)
print("SIGNSPEAK - Quaaf Robust Detection (Fixed Landmark Position)")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_quaaf\quaaf_robust.tflite"
INFO_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_quaaf\quaaf_robust_info.json"
...
# ========== GET REFERENCE QUAAF FROM DATABASE ==========
print("\n📊 Loading reference Quaaf landmarks...")
...
for row in all_data:
    ...
    if label == QUAAF_LETTER:
        features = [float(row[i]) for i in range(1, 43)]
        quaaf_samples.append(features)

quaaf_samples = np.array(quaaf_samples, dtype=np.float32)
quaaf_samples[:, 0::2] = quaaf_samples[:, 0::2] / 300.0
quaaf_samples[:, 1::2] = quaaf_samples[:, 1::2] / 300.0

ref_quaaf = quaaf_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(quaaf_samples)} Quaaf samples")

# ============================================================
# ⭐ FIX: Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_quaaf.min(axis=0)
ref_max = ref_quaaf.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_quaaf_norm = (ref_quaaf - ref_min) / ref_range
...
cv2.namedWindow('Quaaf Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Quaaf Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 QUAAF DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Quaaf (follows your hand)")
print("   GREEN text = Quaaf detected!")
print("")
print("   How to sign Quaaf (ق):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_quaaf_norm - 0.5) * hand_size
            ...
            is_quaaf = smooth_pred > 0.5
            confidence = smooth_pred if is_quaaf else (1 - smooth_pred)
            ...
            if is_quaaf:
                if smooth_pred > 0.8:
                    text = f"✅ QUAAF (ق) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ QUAAF (ق) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ QUAAF (ق) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT QUAAF - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT QUAAF - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Quaaf"
                    color = (0, 165, 255)
            
            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Quaaf Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Quaaf", (45, h-13),
        ...
        ref_ghost = (ref_quaaf_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Quaaf Detection - Robust', frame)
    
    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"quaaf_test_{frame_count}.png"
