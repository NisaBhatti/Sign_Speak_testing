# Save as: testing/test_dal_robust.py
...
print("="*60)
print("SIGNSPEAK - Ḍāl (ڈ) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_dal\dal_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_dal\dal_robust_info.json"
...
# ========== GET REFERENCE DAL FROM DATABASE ==========
print("\n📊 Loading reference Ḍāl landmarks...")
...
for row in all_data:
    ...
    if label == DAL_LETTER:
        ...
        dal_samples.append(features)

if len(dal_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{DAL_LETTER}' (U+0688)!")
    print(f"   The DB may not contain Ḍāl samples.")
    exit(1)

dal_samples = np.array(dal_samples, dtype=np.float32)
dal_samples[:, 0::2] = dal_samples[:, 0::2] / 300.0
dal_samples[:, 1::2] = dal_samples[:, 1::2] / 300.0

ref_dal = dal_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(dal_samples)} Ḍāl samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_dal.min(axis=0)
ref_max = ref_dal.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_dal_norm = (ref_dal - ref_min) / ref_range
...
cv2.namedWindow('Dal Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Dal Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 ḌĀL (ڈ) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Ḍāl (follows your hand)")
print("   GREEN text = Ḍāl detected!")
print("")
print("   How to sign Ḍāl (ڈ):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_dal_norm - 0.5) * hand_size
            ...
            if is_dal:
                if smooth_pred > 0.8:
                    text = f"✅ ḌĀL (ڈ) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ ḌĀL (ڈ) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ ḌĀL (ڈ) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT ḌĀL - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT ḌĀL - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Ḍāl"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Ḍāl Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Ḍāl", (45, h-13),
        ...
        ref_ghost = (ref_dal_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Dal Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"dal_test_{frame_count}.png"
