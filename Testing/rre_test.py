# Save as: testing/test_rre_robust.py
...
print("="*60)
print("SIGNSPEAK - Ṛe (ڑ) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_rre\rre_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_rre\rre_robust_info.json"
...
# ========== GET REFERENCE RRE FROM DATABASE ==========
print("\n📊 Loading reference Ṛe landmarks...")
...
for row in all_data:
    ...
    if label == RRE_LETTER:
        ...
        rre_samples.append(features)

if len(rre_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{RRE_LETTER}' (U+0691)!")
    print(f"   The DB may not contain Ṛe samples.")
    exit(1)

rre_samples = np.array(rre_samples, dtype=np.float32)
rre_samples[:, 0::2] = rre_samples[:, 0::2] / 300.0
rre_samples[:, 1::2] = rre_samples[:, 1::2] / 300.0

ref_rre = rre_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(rre_samples)} Ṛe samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_rre.min(axis=0)
ref_max = ref_rre.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_rre_norm = (ref_rre - ref_min) / ref_range
...
cv2.namedWindow('Rre Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Rre Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 ṚE (ڑ) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Ṛe (follows your hand)")
print("   GREEN text = Ṛe detected!")
print("")
print("   How to sign Ṛe (ڑ):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_rre_norm - 0.5) * hand_size
            ...
            if is_rre:
                if smooth_pred > 0.8:
                    text = f"✅ ṚE (ڑ) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ ṚE (ڑ) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ ṚE (ڑ) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT ṚE - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT ṚE - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Ṛe"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Ṛe Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Ṛe", (45, h-13),
        ...
        ref_ghost = (ref_rre_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Rre Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"rre_test_{frame_count}.png"
