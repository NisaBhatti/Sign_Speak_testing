# Save as: model_training/train_suaad_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Suaad ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_suaad"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Suaad
SUAAD_LETTER = 'ص'
...
# ========== DATA AUGMENTATION FOR SUAAD ==========
print(f"\n🔧 Augmenting Suaad data for robustness...")

suaad_mask = y_all == SUAAD_LETTER
X_suaad_original = X_norm[suaad_mask].copy()
y_suaad_original = y_all[suaad_mask].copy()

print(f"   Original Suaad samples: {len(X_suaad_original)}")

# Create augmented versions
augmented_X = [X_suaad_original]
augmented_y = [y_suaad_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_suaad_original.shape).astype(np.float32)
X_noisy = np.clip(X_suaad_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_suaad_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_suaad_original * scale, 0, 1)
...
for shift_x, shift_y in [(0.02, 0), (-0.02, 0), (0, 0.02), (0, -0.02)]:
    X_shifted = X_suaad_original.copy()
...
X_suaad_augmented = np.vstack(augmented_X)
y_suaad_augmented = np.hstack(augmented_y)

print(f"   Augmented Suaad samples: {len(X_suaad_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~suaad_mask]
y_other = y_all[~suaad_mask]

# Take enough other samples to balance
n_suaad = len(X_suaad_augmented)
n_other = min(len(X_other), n_suaad * 2)
...
# Combine
X = np.vstack([X_suaad_augmented, X_other_bal])
y = np.hstack([np.ones(n_suaad), np.zeros(n_other)])
...
print(f"   Suaad (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_suaad.h5'),
...
# Test on original Suaad samples (not augmented)
suaad_preds = model.predict(X_suaad_original, verbose=0).flatten()
suaad_detected = (suaad_preds > 0.5).sum()
suaad_rate = suaad_detected / len(X_suaad_original) * 100
print(f"\n   Original Suaad detection: {suaad_rate:.1f}% ({suaad_detected}/{len(X_suaad_original)})")
print(f"   Avg Suaad confidence: {suaad_preds.mean():.4f}")
print(f"   Min Suaad confidence: {suaad_preds.min():.4f}")
print(f"   Max Suaad confidence: {suaad_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'suaad_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'suaad_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'suaad_detection_rate': float(suaad_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Suaad', 'Suaad']
}

with open(os.path.join(OUTPUT_DIR, 'suaad_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Suaad Detection: {suaad_rate:.1f}%")