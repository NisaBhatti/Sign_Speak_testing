# Save as: testing/test_daal_robust.py
...
print("="*60)
print("SIGNSPEAK - Dāl (د) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_daal\daal_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_daal\daal_robust_info.json"
...
# ========== GET REFERENCE DAAL FROM DATABASE ==========
print("\n📊 Loading reference Dāl landmarks...")
...
for row in all_data:
    ...
    if label == DAAL_LETTER:
        features = [float(row[i]) for i in range(1, 43)]
        daal_samples.append(features)

if len(daal_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{DAAL_LETTER}' (U+062F)!")
    print(f"   The DB may not contain Dāl samples.")
    exit(1)

daal_samples = np.array(daal_samples, dtype=np.float32)
daal_samples[:, 0::2] = daal_samples[:, 0::2] / 300.0
daal_samples[:, 1::2] = daal_samples[:, 1::2] / 300.0

ref_daal = daal_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(daal_samples)} Dāl samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_daal.min(axis=0)
ref_max = ref_daal.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_daal_norm = (ref_daal - ref_min) / ref_range
...
cv2.namedWindow('Daal Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Daal Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 DĀL (د) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Dāl (follows your hand)")
print("   GREEN text = Dāl detected!")
print("")
print("   How to sign Dāl (د):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_daal_norm - 0.5) * hand_size
            ...
            if is_daal:
                if smooth_pred > 0.8:
                    text = f"✅ DĀL (د) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ DĀL (د) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ DĀL (د) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT DĀL - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT DĀL - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Dāl"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Dāl Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Dāl", (45, h-13),
        ...
        ref_ghost = (ref_daal_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Daal Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"daal_test_{frame_count}.png"
