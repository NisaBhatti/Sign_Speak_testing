# Save as: testing/test_khay_robust.py
...
print("="*60)
print("SIGNSPEAK - Khay (خ) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_khay\khay_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_khay\khay_robust_info.json"
...
# ========== GET REFERENCE KHAY FROM DATABASE ==========
print("\n📊 Loading reference Khay landmarks...")
...
for row in all_data:
    ...
    if label == KHAY_LETTER:
        features = [float(row[i]) for i in range(1, 43)]
        khay_samples.append(features)

if len(khay_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{KHAY_LETTER}' (U+062E)!")
    print(f"   The DB may not contain Khay samples.")
    exit(1)

khay_samples = np.array(khay_samples, dtype=np.float32)
khay_samples[:, 0::2] = khay_samples[:, 0::2] / 300.0
khay_samples[:, 1::2] = khay_samples[:, 1::2] / 300.0

ref_khay = khay_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(khay_samples)} Khay samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_khay.min(axis=0)
ref_max = ref_khay.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_khay_norm = (ref_khay - ref_min) / ref_range
...
cv2.namedWindow('Khay Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Khay Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 KHAY (خ) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Khay (follows your hand)")
print("   GREEN text = Khay detected!")
print("")
print("   How to sign Khay (خ):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_khay_norm - 0.5) * hand_size
            ...
            if is_khay:
                if smooth_pred > 0.8:
                    text = f"✅ KHAY (خ) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ KHAY (خ) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ KHAY (خ) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT KHAY - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT KHAY - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Khay"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Khay Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Khay", (45, h-13),
        ...
        ref_ghost = (ref_khay_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Khay Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"khay_test_{frame_count}.png"
