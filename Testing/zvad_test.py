# Save as: testing/test_zvad_robust.py
...
print("="*60)
print("SIGNSPEAK - Zvād (ض) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_zvad\zvad_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_zvad\zvad_robust_info.json"
...
# ========== GET REFERENCE ZVAD FROM DATABASE ==========
print("\n📊 Loading reference Zvād landmarks...")
...
for row in all_data:
    ...
    if label == ZVAD_LETTER:
        features = [float(row[i]) for i in range(1, 43)]
        zvad_samples.append(features)

if len(zvad_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{ZVAD_LETTER}' (U+0636)!")
    print(f"   The DB may not contain Zvād samples.")
    exit(1)

zvad_samples = np.array(zvad_samples, dtype=np.float32)
zvad_samples[:, 0::2] = zvad_samples[:, 0::2] / 300.0
zvad_samples[:, 1::2] = zvad_samples[:, 1::2] / 300.0

ref_zvad = zvad_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(zvad_samples)} Zvād samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_zvad.min(axis=0)
ref_max = ref_zvad.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_zvad_norm = (ref_zvad - ref_min) / ref_range
...
cv2.namedWindow('Zvad Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Zvad Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 ZVĀD (ض) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Zvād (follows your hand)")
print("   GREEN text = Zvād detected!")
print("")
print("   How to sign Zvād (ض):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_zvad_norm - 0.5) * hand_size
            ...
            if is_zvad:
                if smooth_pred > 0.8:
                    text = f"✅ ZVĀD (ض) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ ZVĀD (ض) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ ZVĀD (ض) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT ZVĀD - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT ZVĀD - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Zvād"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Zvād Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Zvād", (45, h-13),
        ...
        ref_ghost = (ref_zvad_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Zvad Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"zvad_test_{frame_count}.png"
