# Save as: testing/test_lam_robust.py
...
print("="*60)
print("SIGNSPEAK - Lām (ل) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_lam\lam_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_lam\lam_robust_info.json"
...
# ========== GET REFERENCE LAM FROM DATABASE ==========
print("\n📊 Loading reference Lām landmarks...")
...
for row in all_data:
    ...
    if label == LAM_LETTER:
        ...
        lam_samples.append(features)

if len(lam_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{LAM_LETTER}' (U+0644)!")
    print(f"   The DB may not contain Lām samples.")
    exit(1)

lam_samples = np.array(lam_samples, dtype=np.float32)
lam_samples[:, 0::2] = lam_samples[:, 0::2] / 300.0
lam_samples[:, 1::2] = lam_samples[:, 1::2] / 300.0

ref_lam = lam_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(lam_samples)} Lām samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_lam.min(axis=0)
ref_max = ref_lam.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_lam_norm = (ref_lam - ref_min) / ref_range
...
cv2.namedWindow('Lam Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Lam Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 LĀM (ل) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Lām (follows your hand)")
print("   GREEN text = Lām detected!")
print("")
print("   How to sign Lām (ل):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_lam_norm - 0.5) * hand_size
            ...
            if is_lam:
                if smooth_pred > 0.8:
                    text = f"✅ LĀM (ل) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ LĀM (ل) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ LĀM (ل) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT LĀM - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT LĀM - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Lām"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Lām Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Lām", (45, h-13),
        ...
        ref_ghost = (ref_lam_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Lam Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"lam_test_{frame_count}.png"
