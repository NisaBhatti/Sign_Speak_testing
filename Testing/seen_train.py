# Save as: model_training/train_seen_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Seen ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_seen"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Seen
SEEN_LETTER = 'س'
...
# ========== DATA AUGMENTATION FOR SEEN ==========
print(f"\n🔧 Augmenting Seen data for robustness...")

seen_mask = y_all == SEEN_LETTER
X_seen_original = X_norm[seen_mask].copy()
y_seen_original = y_all[seen_mask].copy()

print(f"   Original Seen samples: {len(X_seen_original)}")

# Create augmented versions
augmented_X = [X_seen_original]
augmented_y = [y_seen_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_seen_original.shape).astype(np.float32)
X_noisy = np.clip(X_seen_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_seen_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_seen_original * scale, 0, 1)
...
for shift_x, shift_y in [(0.02, 0), (-0.02, 0), (0, 0.02), (0, -0.02)]:
    X_shifted = X_seen_original.copy()
...
X_seen_augmented = np.vstack(augmented_X)
y_seen_augmented = np.hstack(augmented_y)

print(f"   Augmented Seen samples: {len(X_seen_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~seen_mask]
y_other = y_all[~seen_mask]

# Take enough other samples to balance
n_seen = len(X_seen_augmented)
n_other = min(len(X_other), n_seen * 2)
...
# Combine
X = np.vstack([X_seen_augmented, X_other_bal])
y = np.hstack([np.ones(n_seen), np.zeros(n_other)])
...
print(f"   Seen (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_seen.h5'),
...
# Test on original Seen samples (not augmented)
seen_preds = model.predict(X_seen_original, verbose=0).flatten()
seen_detected = (seen_preds > 0.5).sum()
seen_rate = seen_detected / len(X_seen_original) * 100
print(f"\n   Original Seen detection: {seen_rate:.1f}% ({seen_detected}/{len(X_seen_original)})")
print(f"   Avg Seen confidence: {seen_preds.mean():.4f}")
print(f"   Min Seen confidence: {seen_preds.min():.4f}")
print(f"   Max Seen confidence: {seen_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'seen_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'seen_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'seen_detection_rate': float(seen_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Seen', 'Seen']
}

with open(os.path.join(OUTPUT_DIR, 'seen_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Seen Detection: {seen_rate:.1f}%")