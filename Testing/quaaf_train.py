# Save as: model_training/train_quaaf_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Quaaf ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_quaaf"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Quaaf
QUAAF_LETTER = 'ق'
...
# ========== DATA AUGMENTATION FOR QUAAF ==========
print(f"\n🔧 Augmenting Quaaf data for robustness...")

quaaf_mask = y_all == QUAAF_LETTER
X_quaaf_original = X_norm[quaaf_mask].copy()
y_quaaf_original = y_all[quaaf_mask].copy()

print(f"   Original Quaaf samples: {len(X_quaaf_original)}")

# Create augmented versions
augmented_X = [X_quaaf_original]
augmented_y = [y_quaaf_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_quaaf_original.shape).astype(np.float32)
X_noisy = np.clip(X_quaaf_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_quaaf_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_quaaf_original * scale, 0, 1)
...
for shift_x, shift_y in [(0.02, 0), (-0.02, 0), (0, 0.02), (0, -0.02)]:
    X_shifted = X_quaaf_original.copy()
...
X_quaaf_augmented = np.vstack(augmented_X)
y_quaaf_augmented = np.hstack(augmented_y)

print(f"   Augmented Quaaf samples: {len(X_quaaf_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~quaaf_mask]
y_other = y_all[~quaaf_mask]

# Take enough other samples to balance
n_quaaf = len(X_quaaf_augmented)
n_other = min(len(X_other), n_quaaf * 2)
...
# Combine
X = np.vstack([X_quaaf_augmented, X_other_bal])
y = np.hstack([np.ones(n_quaaf), np.zeros(n_other)])
...
print(f"   Quaaf (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_quaaf.h5'),
...
# Test on original Quaaf samples (not augmented)
quaaf_preds = model.predict(X_quaaf_original, verbose=0).flatten()
quaaf_detected = (quaaf_preds > 0.5).sum()
quaaf_rate = quaaf_detected / len(X_quaaf_original) * 100
print(f"\n   Original Quaaf detection: {quaaf_rate:.1f}% ({quaaf_detected}/{len(X_quaaf_original)})")
print(f"   Avg Quaaf confidence: {quaaf_preds.mean():.4f}")
print(f"   Min Quaaf confidence: {quaaf_preds.min():.4f}")
print(f"   Max Quaaf confidence: {quaaf_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'quaaf_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'quaaf_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'quaaf_detection_rate': float(quaaf_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Quaaf', 'Quaaf']
}

with open(os.path.join(OUTPUT_DIR, 'quaaf_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Quaaf Detection: {quaaf_rate:.1f}%")