# Save as: model_training/train_sheen_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Sheen ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_sheen"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Sheen
SHEEN_LETTER = 'ش'
...
# ========== DATA AUGMENTATION FOR SHEEN ==========
print(f"\n🔧 Augmenting Sheen data for robustness...")

sheen_mask = y_all == SHEEN_LETTER
X_sheen_original = X_norm[sheen_mask].copy()
y_sheen_original = y_all[sheen_mask].copy()

print(f"   Original Sheen samples: {len(X_sheen_original)}")

# Create augmented versions
augmented_X = [X_sheen_original]
augmented_y = [y_sheen_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_sheen_original.shape).astype(np.float32)
X_noisy = np.clip(X_sheen_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_sheen_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_sheen_original * scale, 0, 1)
...
for shift_x, shift_y in [(0.02, 0), (-0.02, 0), (0, 0.02), (0, -0.02)]:
    X_shifted = X_sheen_original.copy()
...
X_sheen_augmented = np.vstack(augmented_X)
y_sheen_augmented = np.hstack(augmented_y)

print(f"   Augmented Sheen samples: {len(X_sheen_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~sheen_mask]
y_other = y_all[~sheen_mask]

# Take enough other samples to balance
n_sheen = len(X_sheen_augmented)
n_other = min(len(X_other), n_sheen * 2)
...
# Combine
X = np.vstack([X_sheen_augmented, X_other_bal])
y = np.hstack([np.ones(n_sheen), np.zeros(n_other)])
...
print(f"   Sheen (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_sheen.h5'),
...
# Test on original Sheen samples (not augmented)
sheen_preds = model.predict(X_sheen_original, verbose=0).flatten()
sheen_detected = (sheen_preds > 0.5).sum()
sheen_rate = sheen_detected / len(X_sheen_original) * 100
print(f"\n   Original Sheen detection: {sheen_rate:.1f}% ({sheen_detected}/{len(X_sheen_original)})")
print(f"   Avg Sheen confidence: {sheen_preds.mean():.4f}")
print(f"   Min Sheen confidence: {sheen_preds.min():.4f}")
print(f"   Max Sheen confidence: {sheen_preds.max():.4f}")
...
# Keras model
model.save(os.path.join(OUTPUT_DIR, 'sheen_robust.h5'))

# TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
with open(os.path.join(OUTPUT_DIR, 'sheen_robust.tflite'), 'wb') as f:
...
info = {
    ...
    'sheen_detection_rate': float(sheen_rate),
    'image_size': f'{IMAGE_WIDTH}x{IMAGE_HEIGHT}',
    'threshold': 0.5,
    'classes': ['Not Sheen', 'Sheen']
}

with open(os.path.join(OUTPUT_DIR, 'sheen_robust_info.json'), 'w', encoding='utf-8') as f:
    json.dump(info, f, indent=2, ensure_ascii=False)

print(f"\n✅ TRAINING COMPLETE!")
print(f"   Test Accuracy: {test_acc*100:.2f}%")
print(f"   Sheen Detection: {sheen_rate:.1f}%")