# Save as: testing/test_gaf_robust.py
...
print("="*60)
print("SIGNSPEAK - Gāf (گ) Robust Detection")
print("="*60)

# ========== PATHS ==========
MODEL_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_gaf\gaf_robust.tflite"
INFO_PATH  = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_gaf\gaf_robust_info.json"
...
# ========== GET REFERENCE GAF FROM DATABASE ==========
print("\n📊 Loading reference Gāf landmarks...")
...
for row in all_data:
    ...
    if label == GAF_LETTER:
        ...
        gaf_samples.append(features)

if len(gaf_samples) == 0:
    print(f"\n❌ ERROR: No reference samples found for '{GAF_LETTER}' (U+06AF)!")
    print(f"   The DB may not contain Gāf samples.")
    exit(1)

gaf_samples = np.array(gaf_samples, dtype=np.float32)
gaf_samples[:, 0::2] = gaf_samples[:, 0::2] / 300.0
gaf_samples[:, 1::2] = gaf_samples[:, 1::2] / 300.0

ref_gaf = gaf_samples.mean(axis=0).reshape(21, 2)
print(f"   Reference from {len(gaf_samples)} Gāf samples")

# ============================================================
# ⭐ Normalize reference to 0-1 bounding box
# ============================================================
ref_min = ref_gaf.min(axis=0)
ref_max = ref_gaf.max(axis=0)
ref_range = np.maximum(ref_max - ref_min, 1e-6)
ref_gaf_norm = (ref_gaf - ref_min) / ref_range
...
cv2.namedWindow('Gaf Detection - Robust', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Gaf Detection - Robust', 1100, 800)

print("\n" + "="*60)
print("🎯 GĀF (گ) DETECTION")
print("="*60)
print("   GREEN dots = YOUR hand")
print("   WHITE skeleton = Reference Gāf (follows your hand)")
print("   GREEN text = Gāf detected!")
print("")
print("   How to sign Gāf (گ):")
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ref_world = (ref_gaf_norm - 0.5) * hand_size
            ...
            if is_gaf:
                if smooth_pred > 0.8:
                    text = f"✅ GĀF (گ) - HIGH CONFIDENCE!"
                    color = (0, 255, 0)
                elif smooth_pred > 0.6:
                    text = f"✅ GĀF (گ) - Good"
                    color = (0, 255, 128)
                else:
                    text = f"✅ GĀF (گ) - Low confidence"
                    color = (0, 255, 255)
            else:
                if smooth_pred < 0.2:
                    text = f"❌ NOT GĀF - Very different"
                    color = (0, 0, 255)
                elif smooth_pred < 0.4:
                    text = f"❌ NOT GĀF - Different"
                    color = (0, 128, 255)
                else:
                    text = f"⚠️ UNCERTAIN - Close to Gāf"
                    color = (0, 165, 255)

            cv2.putText(frame, text, (20, 35),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.putText(frame, f"Gāf Score: {smooth_pred*100:.1f}%", (20, 65),
            ...
            cv2.putText(frame, "Reference Gāf", (45, h-13),
        ...
        ref_ghost = (ref_gaf_norm - 0.5) * ghost_size + ghost_center
        for conn in mp_hands.HAND_CONNECTIONS:
            s = ref_ghost[conn[0]]
            e = ref_ghost[conn[1]]
            cv2.line(frame,
    ...
    cv2.imshow('Gaf Detection - Robust', frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    elif key == ord('s'):
        filename = f"gaf_test_{frame_count}.png"
