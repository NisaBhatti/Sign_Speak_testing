# Save as: model_training/train_tay_robust.py
...
print("="*60)
print("SIGNSPEAK - Training Tay ROBUST Model")
print("="*60)

# ========== CONFIGURATION ==========
DB_PATH = r"C:\Users\asifa\OneDrive\Desktop\Model\Simple_Dataset\main_dataset.db"
OUTPUT_DIR = r"C:\Users\asifa\OneDrive\Desktop\Model\Exported_Model\exported_models_tay"
os.makedirs(OUTPUT_DIR, exist_ok=True)

IMAGE_WIDTH = 300
IMAGE_HEIGHT = 300

# Arabic letter for Tay
TAY_LETTER = 'ط'
...
# ========== DATA AUGMENTATION FOR TAY ==========
print(f"\n🔧 Augmenting Tay data for robustness...")

tay_mask = y_all == TAY_LETTER
X_tay_original = X_norm[tay_mask].copy()
y_tay_original = y_all[tay_mask].copy()

print(f"   Original Tay samples: {len(X_tay_original)}")

# Create augmented versions
augmented_X = [X_tay_original]
augmented_y = [y_tay_original]

# Augmentation 1: Add small random noise (simulates camera noise)
noise = np.random.normal(0, 0.02, X_tay_original.shape).astype(np.float32)
X_noisy = np.clip(X_tay_original + noise, 0, 1)
augmented_X.append(X_noisy)
augmented_y.append(y_tay_original)

# Augmentation 2: Scale slightly (simulates different distances)
for scale in [0.9, 1.1]:
    X_scaled = np.clip(X_tay_original * scale, 0, 1)
...
X_tay_augmented = np.vstack(augmented_X)
y_tay_augmented = np.hstack(augmented_y)

print(f"   Augmented Tay samples: {len(X_tay_augmented)}")

# ========== CREATE DATASET ==========
print(f"\n📊 Creating balanced dataset...")

# Get other class samples
X_other = X_norm[~tay_mask]
y_other = y_all[~tay_mask]

# Take enough other samples to balance
n_tay = len(X_tay_augmented)
...
# Combine
X = np.vstack([X_tay_augmented, X_other_bal])
y = np.hstack([np.ones(n_tay), np.zeros(n_other)])
...
print(f"   Tay (1): {int(y.sum())} | Other (0): {len(y)-int(y.sum())}")
...
callbacks = [
    ...
    ModelCheckpoint(
        os.path.join(OUTPUT_DIR, 'best_tay.h5'),
...
# Test on original Tay samples (not augmented)
tay_preds = model.predict(X_tay_original, verbose=0).flatten()
tay_detected = (tay_preds > 0.5).sum()
tay_rate = tay_detected / len(X_tay_original) * 100
print(f"\n   Original Tay detection: {tay_rate:.1f}% ({tay_detected}/{len(X_tay_original)})")
print(f"   Avg Tay confidence: {tay_preds.mean():.4f}")
print(f"   Min Tay confidence: {tay_preds.min():.4f}")
print(f"   Max Tay confidence: {tay_preds.max():.4f}")
...
print(f"   Tay Detection: {tay_rate:.1f}%")